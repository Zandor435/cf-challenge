...
- **SP+ win-prob scale — HELD at `WIN_PROB_POINTS_SCALE=11.0`, `HOME_FIELD_ADVANTAGE_PTS=2.5` (DECIDED 2026-07). Do NOT adopt calibrate.py's fitted values.** The scale cannot be validated non-leakily: CFBD's `/ratings/sp` (and `/ratings/fpi`) return **season-FINAL** ratings only — the `week` param is accepted but ignored (verified: Indiana 2024 SP+ = 20.1 at no-week / week=3 / week=10; 2025 cache == live == week=4). A calibration off final ratings is **hindsight-leaked** — it predicts early-week games with ratings that already encode the season's results, biasing the fitted scale too steep (→ overconfident live). `calibrate.py` therefore ships a leakage warning and its output is a *leaky diagnostic*, not a recommendation. The **only** vintage-correct in-season rating in CFBD is **Elo** (`/ratings/elo?year=&week=&seasonType=`, verified week-varying: Indiana 2024 = 1527/1875/1954 at wk3/10/15). A proper (holdout, vintage) calibration is possible **only** by switching the spine SP+→Elo — a deliberate reopen of the ratings decision, not a constant tweak. FPI A/B has the same leakage limitation and is likewise shelved.
