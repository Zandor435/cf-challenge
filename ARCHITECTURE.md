...
- **Head-to-head games are ONE draw (LOCKED).** Sharing a draw per *team* is necessary but not sufficient: when two **picked teams play each other**, the same physical game sits on both slates, and drawing each team's schedule independently lets a trial contain **both teams winning it** — an outcome that does not exist. At week 6 the four groups carry 5 / 10 / 17 / 28 such games, so this is routine, not a corner. `simulate_game_wins()` therefore draws **one uniform per game**: games pair on `(week, {team_a, team_b})`, the **reference side** (home team; alphabetically-first canonical name at a neutral site) wins iff `u < p_reference`, and the other side wins iff the reference lost. The two sides' probabilities are already exact complements — HFA is antisymmetric and `logistic(-x) = 1 - logistic(x)` — so nothing is reconciled; `p_A + p_B == 1` is **asserted**, and a slate asymmetry (A lists B but B does not list A) **fails loud naming both teams**. Coupling changes the **joint** only: every per-pick marginal is arithmetically unchanged (zero-diff across all four groups), and only `p05/p50/p95` and `p_win_pool` move. `test_projector_h2h.py` asserts coherence, complementarity, marginal preservation, and the loud failure.
