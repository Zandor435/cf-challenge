...
- **Coach profile art — LOCATION LOGGED (2026-08-15). Lives on Zach's Windows box, NOT in this repo.** Root: `C:\Users\zacha\Claude Code\cf-challenge\output\personas\`. The set flagged in this thread: `...\output\personas\Fat friends\Fat` — recorded verbatim as given; the trailing leaf looks truncated, so confirm the exact folder name before anything scripts against it. Note `Fat friends` matches no `groups/` slug (`family` / `church` / `panel`), so it is not a per-group output directory. Consequences:
  - Nothing in CI or the pipeline reads that path — `output/` is not tracked here and is not a build input. Do **not** add a build step that reaches onto a local drive; selected profiles get copied in by hand.
  - The design-lab rule stands: the persona pipeline is the art source, and every variant reserves a 5:7 portrait or 4:3 block for it in `--cf-muted-gold` (`docs/design-lab/NOTES.md`, "Open questions" item 5). Nothing invents a competing illustration style.
- **Coach profiles on the site — WIRED (2026-08-15). The four profiles in that folder are assigned to `panel`.** Destination `docs/assets/portraits/<group_id>/`, mapped by `docs/assets/portraits/manifest.json` (`group_id → manager_id → file`); `docs/app.js` renders the portrait inside the manager avatar disc at all three sizes. See `docs/assets/portraits/README.md`.
  - **Site assets, not engine output.** `scripts/` neither reads nor regenerates them and `docs/output-contract.md` does not cover them — the contract's write surface is still exactly the three JSON files per group.
  - **Three silent tiers:** listed + loads → portrait; not listed → initials (unchanged behaviour, which is what `family` and `church` still get); listed but missing/404 → initials via the `error` handler. So a wrong extension is indistinguishable from "no portrait yet" — check `manifest.json` against disk when one doesn't appear.
  - The manifest already lists all four Panel managers at `panel/<manager_id>.png` (`blaine`, `chris`, `jonathan`, `zach`), so dropping four correctly-named files in needs no code or config edit. Mapping is by **`manager_id`**, never `display_name`.
  - **`scripts/make_avatars.py`** does the copy-in: crop square (anchored high, matching the CSS), resize to 256px, name by `manager_id`, refresh the manifest. Hand-run only — no workflow calls it, and `pillow` is in `requirements.txt` for it alone. It **refuses to guess** an ambiguous filename→manager mapping (rule 4) and writes nothing unless every manager resolves (rule 5); `--map` states the pairing explicitly, `--allow-partial` opts out of the second guard.
  - **Still open:** the `PROFILES` nav tab is a coming-soon placeholder. When it gets built it should read the same manifest rather than growing a second art path, and that is where the 5:7 dossier crop earns its keep — the circles crop `center top` today.
