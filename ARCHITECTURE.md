...
- **SP+ win-prob scale `WIN_PROB_POINTS_SCALE` — OPEN. The constant is UNVALIDATED in both directions; no value is decided.** The projector currently *holds* `11.0` / `HOME_FIELD_ADVANTAGE_PTS=2.5`, but "hold" is a placeholder pending investigation, not a verdict. Two candidate numbers are on record and **neither is validated**:
  - `7.1` is **leak-contaminated**. CFBD's `/ratings/sp` (and `/ratings/fpi`) return **season-FINAL** ratings only — the `week` param is accepted but ignored (verified: Indiana 2024 SP+ = 20.1 at no-week / week=3 / week=10; 2025 cache == live == week=4). `calibrate.py`'s fit predicts early-week games with ratings that already encode the season's results, biasing the scale **too steep** (→ overconfident live). It is a *leaky diagnostic*, not a recommendation.
  - `11.0` is **inherited from the wc-challenge repo** (different sport, different rating units) and has **never been tested against anything** in this project. Its only claim to fitness was that it wasn't the leaky number.
  A non-leaky calibration is **not** impossible — it is impossible only *using in-season SP+*. Two leak-free designs (implemented in `scripts/calibrate_spread.py`, written up in `docs/calibration-report.md`, run 2026-07 on 2021–2025) put the true SP+ scale in the bracket **7.95 ≤ scale ≤ 17.45**; a market-derived (closing-spread) bridge lands it near **~11.9**. On that evidence **`11.0` sits inside the bracket and the leaky `7.1` sits outside it (too steep)** — but the band is wide, so `11.0` is *defensible, not validated to a point*. **Zach decides whether to keep 11.0, move toward the market-implied ~12, or reopen the spine.** The fully vintage-correct in-season rating in CFBD is **Elo** (`/ratings/elo?year=&week=&seasonType=`, week-varying: Indiana 2024 = 1527/1875/1954 at wk3/10/15); a within-season SP+ holdout is possible only by switching the spine SP+→Elo (a deliberate ratings reopen, not a constant tweak). FPI A/B has the same in-season leakage limitation and is likewise shelved. **HFA 2.5 is also open** — every leak-free estimate (market intercept ~2.7, attenuation-implied ~3.9, prior-season fit 5.45) sits at or above it, a mild case for a nudge toward ~3.
